...
class Mapping_hafalan_hadist extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('M_mapping_hafalan_hadist');
	...
	function index(){
		$this->breadcrumb->add('Settings','mapping/mapping_hafalan_hadist')
		->add('Data Ayat','mapping/mapping_hafalan_hadist');
		...
		$data = array(
			'thisContent'		=> 'mapping/v_mapping_hafalan_hadist',
			'thisJs'			=> 'mapping/js_mapping_hafalan_hadist',
	...
	function list_data(){
		...
		$data = $this->M_mapping_hafalan_hadist->get_data_mappingHafalan($jurusan,$angkatan);
		echo $data;
	}
	...
	function save(){
		...
		if ($id == 0) {
			$insert = $this->M_mapping_hafalan_hadist->insert($item);
			$result['feedback'] = 'Successfully Add Mapping'; 
		} else {
			$this->M_mapping_hafalan_hadist->update($id, $item);
	...
	public function uploads(){
		...
		if ($this->upload->do_upload('fileupload')){
			...
			$ins = $this->M_mapping_hafalan_hadist->insert_batch($items);
			if($ins){
	...
	function delete(){
		$id = $_POST['id'];
		$this->M_mapping_hafalan_hadist->delete($id);
	}
