...
class Mapping_hafalan_ayat extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('M_mapping_hafalan_ayat');
	...
	function index(){
		$this->breadcrumb->add('Settings','mapping/mapping_hafalan_ayat')
		->add('Data Ayat','mapping/mapping_hafalan_ayat');
		...
		$data = array(
			'thisContent'		=> 'mapping/v_mapping_hafalan_ayat',
			'thisJs'			=> 'mapping/js_mapping_hafalan_ayat',
	...
	function list_data(){
		...
		$data = $this->M_mapping_hafalan_ayat->get_data_mappingHafalan($jurusan,$angkatan);
		echo $data;
	}
	...
	function save(){
		...
		if ($id == 0) {
			$insert = $this->M_mapping_hafalan_ayat->insert($item);
			$result['feedback'] = 'Successfully Add Mapping'; 
		} else {
			$this->M_mapping_hafalan_ayat->update($id, $item);
	...
	public function uploads(){
		...
		if ($this->upload->do_upload('fileupload')){
			...
			$ins = $this->M_mapping_hafalan_ayat->insert_batch($items);
			if($ins){
	...
	function delete(){
		$id = $_POST['id'];
		$this->M_mapping_hafalan_ayat->delete($id);
	}
