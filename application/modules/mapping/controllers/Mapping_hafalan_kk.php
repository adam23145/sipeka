...
class Mapping_hafalan_kk extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('M_mapping_hafalan_kk');
	...
	function index(){
		$this->breadcrumb->add('Settings','mapping/mapping_hafalan_kk')
		->add('Data Ayat','mapping/mapping_hafalan_kk');
		...
		$data = array(
			'thisContent'		=> 'mapping/v_mapping_hafalan_kk',
			'thisJs'			=> 'mapping/js_mapping_hafalan_kk',
	...
	function list_data(){
		...
		$data = $this->M_mapping_hafalan_kk->get_data_mappingHafalan($jurusan,$angkatan);
		echo $data;
	}
	...
	function save(){
		...
		$item = array(
			'nim'				=> $_POST['nim'],
			'kiraatul_kutub'	=> $_POST['kiraatul_kutub'],
			'nip'				=> $_POST['nip'],
			'upd'				=> $this->session->userdata['logged_in']['userid']
		);		
		if ($id == 0) {
			$insert = $this->M_mapping_hafalan_kk->insert($item);
			$result['feedback'] = 'Successfully Add Mapping'; 
		} else {
			$this->M_mapping_hafalan_kk->update($id, $item);
		...
		if ($this->upload->do_upload('fileupload')){
			...
			foreach( $schdeules as $single_schedule )
			{
				if($i > 0){
					...
					$vars['kiraatul_kutub']	= $single_schedule[1];
			...
			$ins = $this->M_mapping_hafalan_kk->insert_batch($items);
			if($ins){
	...
	function delete(){
		$id = $_POST['id'];
		$this->M_mapping_hafalan_kk->delete($id);
	}
