 ...
 * 
 */
class M_mapping_hafalan_kk extends CI_Model {
	...
	function get_data_mappingHafalan($jurusan,$angkatan) {
		...
		$this->datatables->select('
			...
			mh.nama mhs,
			ma.kiraotul_kutub nama_kk,
			ma.tema,
			md.nama dosen
		');
		$this->datatables->from('mapping_hafalan_kk h');
		...
		$this->datatables->join('m_kk ma','ma."id" = h.kiraatul_kutub','inner');
	...
	function update($id, $item){
		$this->db->where('id',$id);
		$update = $this->db->update('mapping_hafalan_kk',$item);
		return $update;
	}

	function insert($item){
		$insert 	= $this->db->insert('mapping_hafalan_kk',$item);
		return $insert;
	}
	
	function insert_batch($item){
		$insert 	= $this->db->insert_batch('mapping_hafalan_kk',$item);
		return $insert;
	}
	...
	$this->db->where('id', $id);
	$delete = $this->db->delete('mapping_hafalan_kk');
	return $delete;
