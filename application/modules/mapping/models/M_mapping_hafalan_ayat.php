 ...
 * 
 */
class M_mapping_hafalan_ayat extends CI_Model
{
	...
	function get_data_mappingHafalan($jurusan, $angkatan)
	{
		...
		$this->datatables->select('
			...
			md.nama dosen
			');
		$this->datatables->from('mapping_hafalan_ayat h');
		...
		$this->datatables->join('m_ayat ma', 'ma."id" = h.ayat', 'inner');
		...
		$this->datatables->where('ma.status = 1');
		if ($jurusan <> "") {
			$this->datatables->where("ma.id_prodi = '$jurusan'");
		}
		if ($angkatan <> "") {
			$this->datatables->where("mh.tahun_masuk = '$angkatan'");
		}
	...
	{
		$this->db->where('id', $id);
		$update = $this->db->update('mapping_hafalan_ayat', $item);
		return $update;
	}

	function insert($item)
	{
		$insert 	= $this->db->insert('mapping_hafalan_ayat', $item);
		return $insert;
	}

	function insert_batch($item)
	{
		$insert 	= $this->db->insert_batch('mapping_hafalan_ayat', $item);
		return $insert;
	}
	...
	function delete($id)
	{
		...
		$this->db->where('id', $id);
		$delete = $this->db->delete('mapping_hafalan_ayat');
		return $delete;
	}
}
