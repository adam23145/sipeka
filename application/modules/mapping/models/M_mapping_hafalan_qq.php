 ...
 * 
 */
class M_mapping_hafalan_qq extends CI_Model {
	...
	function get_data_mappingHafalan($jurusan,$angkatan) {
		...
		$this->datatables->select('
			...
			mh.nama mhs,
			ma.kiraatul_quran nama_qq,
			ma.tema,
			md.nama dosen
		');
		$this->datatables->from('mapping_hafalan_qq h');
		...
		$this->datatables->join('m_qq ma','ma."id" = h.kiraatul_quran','inner');
	...
	function update($id, $item){
		$this->db->where('id',$id);
		$update = $this->db->update('mapping_hafalan_qq',$item);
		return $update;
	}

	function insert($item){
		$insert 	= $this->db->insert('mapping_hafalan_qq',$item);
		return $insert;
	}
	
	function insert_batch($item){
		$insert 	= $this->db->insert_batch('mapping_hafalan_qq',$item);
		return $insert;
	}
	...
	$this->db->where('id', $id);
	$delete = $this->db->delete('mapping_hafalan_qq');
	return $delete;
