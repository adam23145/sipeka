<script type="text/javascript">
	...
	$("#tableTransHafalan").DataTable({
		...
		// lengthChange: false,
		ajax: {
			url: baseURL + "hafalan/hafalan_kk/list_data",
			type: "POST",
			data: function(d){
	...
	$('#tableTransHafalan').on('click', '.btn-edit', function(){
		...
		id 					= data['id'];
		mapping_kk			= data['mapping_kk'];
		kiraatul_kutub		= data['kiraatul_kutub'];
		tema 				= data['tema'];
		link 				= data['link'];

		$('#id').val(id);
		$('#mapping_kk').val(mapping_kk);
		$('#kiraatul_kutub').html(kiraatul_kutub);
		$('#tema').html("Tema - "+tema);
		$('#link').val(link);
	...
	$('#tableTransHafalan').on('click', '.btn-nilai', function(){
		...
		$('#idForDosen').val(id);
		$('#mapping_kk').val(mapping_kk);
		$('#kkForDosen').html(kiraatul_kutub);
		$('#temaForDosen').html("Tema - "+tema);
		$('#link_dosen').val(link);
		$('#membaca').val(membaca);
		$('#memahami').val(memahami);
		$('#menganalisa').val(menganalisa);
		$('#nilai').val(nilai);
	...
	$('#form-inputLink').submit(function(e){
		...
		$.ajax({
			url: baseURL + 'hafalan/hafalan_kk/save',
			type: 'POST',
			data: $(this).serialize(),
			dataType: 'JSON',
	...
	$('#form-inputNilai').submit(function(e){
		...
		$.ajax({
			url: baseURL + 'hafalan/hafalan_kk/save_nilai',
			type: 'POST',
			data: data,
			dataType: 'JSON',
			success: function(data){
