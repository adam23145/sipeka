<script type="text/javascript">
	...
	$("#tableTransHafalan").DataTable({
		...
		// lengthChange: false,
		ajax: {
			url: baseURL + "hafalan/hafalan_ayat/list_data",
			type: "POST",
			data: function(d){
		...
		rowCallback: function (row, data, iDisplayIndex) {
			...
			if("<?php echo $level;?>" == "mahasiswa"){
				if(data["link"] === null){
					$(row).css('background-color', '#e7e8ab');
	...
	$('#tableTransHafalan').on('click', '.btn-edit', function(){
		...
		id 					= data['id'];
		mapping_ayat		= data['mapping_ayat'];
		ayat 				= data['ayat'];
		tema 				= data['tema'];
		link 				= data['link'];

		$('#id').val(id);
		$('#mapping_ayat').val(mapping_ayat);
		$('#ayat').html(ayat);
		$('#tema').html("Tema - "+tema);
		$('#link').val(link);
	...
	$('#form-inputLink').submit(function(e){
		...
		$.ajax({
			url: baseURL + 'hafalan/hafalan_ayat/save',
			type: 'POST',
			data: $(this).serialize(),
			dataType: 'JSON',
	...
	$('#form-inputNilai').submit(function(e){
		...
		$.ajax({
			url: baseURL + 'hafalan/hafalan_ayat/save_nilai',
			type: 'POST',
			data: data,
			dataType: 'JSON',
			success: function(data){
