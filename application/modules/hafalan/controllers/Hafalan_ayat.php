...
class Hafalan_ayat extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('M_hafalan_ayat');
	...
	function index(){
		$this->breadcrumb->add('Hafalan','hafalan/hafalan_ayat')
		->add('Data Ayat','hafalan/hafalan_ayat');
		$data_mahasiswa = $this->M_global->get_mahasiswa();
		$data_ayat = $this->M_global->get_ayat();
		$data_dosen = $this->M_global->get_dosen1();
		$data_status = $this->M_global->get_status();
		$level = $this->session->userdata['logged_in']['userlevel'];
		$data = array(
			'thisContent'		=> 'hafalan/v_hafalan_ayat',
			'thisJs'			=> 'hafalan/js_hafalan_ayat',
			'level'				=> $level,
	...
	function list_data(){
		...
		$data = $this->M_hafalan_ayat->get_data_hafalan($this->session->userdata['logged_in']['userid'],$this->session->userdata['logged_in']['userlevel'],$post['search']['value']);
		echo $data;
	}
	...
	function save(){
		$id	= $_POST['id'];
		$item = array(
			'mapping_ayat'	=> $_POST['mapping_ayat'],
			'link'			=> $_POST['link'],
			'tgl_upload'	=> date("Y-m-d H:i:s"),
			'upd'			=> $this->session->userdata['logged_in']['userid']
		);
		if ($id == 0 or $id == ""){
			$insert = $this->M_hafalan_ayat->insert($item);
			$result['feedback'] = 'Successfully Add Link'; 
		} else {
			$this->M_hafalan_ayat->update($id, $item);
	...
	public function uploads(){
		...
		if ($this->upload->do_upload('fileupload')){
			...
			$ins = $this->M_hafalan_ayat->insert_batch($items);
			if($ins){
	...
	function cetak_pdf(){
		...
		$get_bgimage = file_get_contents(FCPATH . "public/assets/core/images/bg_sertifikat.png");
		$get_image = file_get_contents(FCPATH . "public/assets/core/images/logo.png");
		$nim = explode("@", $this->session->userdata['logged_in']['userid']);
		$get_user		= $this->M_hafalan_ayat->getdata_sertifikat($nim[0]);
	    ...
	    $html = $this->load->view('../modules/hafalan/views/sertifikat_ayat', $data, TRUE);
	    // $this->load->view('../modules/hafalan/views/sertifikat_ayat', $data, TRUE);
