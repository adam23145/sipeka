<script type="text/javascript">
	...
	$("#table-skripsi").DataTable({
		...
		ajax: {
			url: baseURL + "history/bimbingan_skripsi/data_submission",
			type: "POST",
			data: function(d){
	...
	$('#table-skripsi').on('click', '.btn-view', function(){
		currentRow 			= $(this).closest('tr');
		data 				= $('#table-skripsi').DataTable().row(currentRow).data();
		id 					= data['id'];
		submission_code 	= data['submission_code'];
		title 				= data['title'];
		rms_maslh 			= data['rms_maslh'];

		var url = baseURL+'history/form_detail/edit/'+submission_code;
