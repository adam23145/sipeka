 ...
 * 
 */
class M_hadist extends CI_Model {

	function get_data_hadist($jurusan) {
		$i=0;
		$this->datatables->select('
			a.id,
			ROW_NUMBER() OVER (ORDER BY a.hadist) AS no_urut,
			a.id_prodi,
			s.id id_status,
			a.hadist,
			a.tema,
			...
			j.major_name
			');
		$this->datatables->from('m_hadist a');
	...
	function update($id, $item){
		$this->db->where('id',$id);
		$update = $this->db->update('m_hadist',$item);
		return $update;
	}

	function insert($item){
		$insert 	= $this->db->insert('m_hadist',$item);
		return $insert;
	}
	
	function insert_batch($item){
		$insert 	= $this->db->insert_batch('m_hadist',$item);
		return $insert;
	}
	...
	$this->db->where('id', $id);
	$delete = $this->db->delete('m_hadist');
	return $delete;
