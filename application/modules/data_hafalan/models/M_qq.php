 ...
 * 
 */
class M_qq extends CI_Model {

	function get_data_qq($jurusan) {
		$i=0;
		$this->datatables->select('
			a.id,
			ROW_NUMBER() OVER (ORDER BY a.kiraatul_quran) AS no_urut,
			a.id_prodi,
			s.id id_status,
			a.kiraatul_quran,
			a.tema,
	...
	function update($id, $item){
		$this->db->where('id',$id);
		$update = $this->db->update('m_qq',$item);
		return $update;
	}

	function insert($item){
		$insert 	= $this->db->insert('m_qq',$item);
		return $insert;
	}
	
	function insert_batch($item){
		$insert 	= $this->db->insert_batch('m_qq',$item);
		return $insert;
	}
