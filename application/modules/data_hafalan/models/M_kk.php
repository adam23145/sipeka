...
class M_kk extends CI_Model {
	...
	function get_data_kk($jurusan) {
		$i=0;
		$this->datatables->select('
			a.id,
			ROW_NUMBER() OVER (ORDER BY a.kiraotul_kutub) AS no_urut,
			a.id_prodi,
			s.id id_status,
			a.kiraotul_kutub,
			a.tema,
	...
	function update($id, $item){
		$this->db->where('id',$id);
		$update = $this->db->update('m_kk',$item);
		return $update;
	}

	function insert($item){
		$insert 	= $this->db->insert('m_kk',$item);
		return $insert;
	}
	
	function insert_batch($item){
		$insert 	= $this->db->insert_batch('m_kk',$item);
		return $insert;
	}
