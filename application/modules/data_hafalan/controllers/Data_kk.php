...
class Data_kk extends CI_Controller {
	...
	function index(){
		$this->breadcrumb->add('Settings','data_hafalan/data_kk')
		->add('Data Kiratul Kutub','data_hafalan/data_kk');
		$data_prodi = $this->M_global->get_prodi();
		$data_status = $this->M_global->get_status();
		// $data_tahun = $this->M_global->get_start_angkatan();
		$data = array(
			'thisContent' 	=> 'data_hafalan/v_kk',
			'thisJs'		=> 'data_hafalan/js_data_kk',
	...
	function list_data(){
		...
		$data = $this->M_kk->get_data_kk($jurusan);
		echo $data;
	}
	...
	function save(){
		...
		if ($id == 0) {
			$insert = $this->M_kk->insert($item);
			$result['feedback'] = 'Successfully Add Kiraotul Kutub'; 
		} else {
			$this->M_kk->update($id, $item);
			$result['feedback'] = 'Successfully Update Kiraotul Kutub'; 
		}
	...
	public function uploads(){
		...
		if ($this->upload->do_upload('fileupload')){
			...
			foreach( $schdeules as $single_schedule )
			{
				if($i > 0){
					// $j=0;
					$vars['kiraotul_kutub']	= $single_schedule[0];
			...
			$ins = $this->M_kk->insert_batch($items);
			if($ins){
