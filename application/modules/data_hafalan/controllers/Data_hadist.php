...
class Data_hadist extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('M_hadist');
	...
	function index(){
		$this->breadcrumb->add('Settings','data_hafalan/data_hadist')
		->add('Data Hadist','data_hafalan/data_hadist');
		$data_prodi = $this->M_global->get_prodi();
		$data_status = $this->M_global->get_status();
		// $data_tahun = $this->M_global->get_start_angkatan();
		$data = array(
			'thisContent' 	=> 'data_hafalan/v_hadist',
			'thisJs'		=> 'data_hafalan/js_data_hadist',
	...
	function list_data(){
		...
		$data = $this->M_hadist->get_data_hadist($jurusan);
		echo $data;
	}
	...
	function save(){
		...
		if ($id == 0) {
			$insert = $this->M_hadist->insert($item);
			$result['feedback'] = 'Successfully Added Login'; 
		} else {
			$this->M_hadist->update($id, $item);
	...
	public function uploads(){
		...
		if ($this->upload->do_upload('fileupload')){
			...
			$ins = $this->M_hadist->insert_batch($items);
			if($ins){
