...
class Data_qq extends CI_Controller {

	function __construct(){
		parent::__construct();
		$this->load->model('M_qq');
	...
	function index(){
		$this->breadcrumb->add('Settings','data_hafalan/data_qq')
		->add('Data Kiraatul Quran','data_hafalan/data_qq');
		$data_prodi = $this->M_global->get_prodi();
		$data_status = $this->M_global->get_status();
		// $data_tahun = $this->M_global->get_start_angkatan();
		$data = array(
			'thisContent' 	=> 'data_hafalan/v_qq',
			'thisJs'		=> 'data_hafalan/js_data_qq',
	...
	function list_data(){
		...
		$data = $this->M_qq->get_data_qq($jurusan);
		echo $data;
	}
	...
	function save(){
		...
		if ($id == 0) {
			$insert = $this->M_qq->insert($item);
			$result['feedback'] = 'Successfully Add Kiraatul Qur`an'; 
		} else {
			$this->M_qq->update($id, $item);
			$result['feedback'] = 'Successfully Update Kiraotul Qur`an'; 
		}
	...
	public function uploads(){
		...
		if ($this->upload->do_upload('fileupload')){
			...
			{
				if($i > 0){
					$vars['kiraatul_quran']	= $single_schedule[0];
			...
			$ins = $this->M_qq->insert_batch($items);
			if($ins){
