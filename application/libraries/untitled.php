...
class Breadcrumb {
	...
	public function show($home_site ="home", $id = "crumbs") {
		...
		$out  = '<ol class="breadcrumb" id="'.$id.'">
		';
		if ($breadcrumbs && count($breadcrumbs) > 0) {
			$out .= '<li class="breadcrumb-item"><a href="#"><i class="icon dripicons-home"></i></a></li>';
			$i=1;
			foreach ($breadcrumbs as $crumb): 
				...
				if ($i != count($breadcrumbs)) {
					...
					$out .=  '<li class="breadcrumb-item"><a href="#">'. $crumb['title'] .'</a></li>';
				}
				$i++;
			endforeach;
		} else {
